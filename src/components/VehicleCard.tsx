...
export function VehicleCard({ vehicle, highlight = false }: { vehicle: Vehicle; highlight?: boolean }) {
  ...
  return <article className={`group flex h-full flex-col overflow-hidden rounded-[2rem] border bg-inverse text-inverse-foreground transition duration-300 hover:-translate-y-1 hover:shadow-2xl ${highlight ? 'border-primary ring-2 ring-primary/30' : 'border-border'}`}>
    ...
    <div className="flex flex-1 flex-col p-5 sm:p-6">
      ...
      <div className="mt-auto flex items-end justify-between gap-3 pt-6">
        <strong className="font-display text-2xl leading-none sm:text-3xl">{money(vehicle.price)}</strong>
