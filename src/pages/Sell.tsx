...
export default function Sell(){
  ...
  const submit=async(v:Values)=>{if(busy)return;setBusy(true);setError('');try{
    ...
    const url=whatsappFor(`Olá! Enviei uma proposta de venda/consignação: ${v.brand} ${v.model} ${v.year}, ${v.mileage} km. Meu nome é ${v.name}. Protocolo: ${id}`);setWhatsappUrl(url);setSuccess(true);window.open(url,'_blank','noopener,noreferrer')
  }catch(e){setError(e instanceof Error?e.message:'Não foi possível enviar. Tente novamente.')}finally{setBusy(false)}}
  return <><SEO title="Venda seu veículo" description="Venda ou consigne seu carro ou moto com a Braza Veículos em Uberlândia."/><section className="container-wide pb-24 pt-40"><div className="max-w-3xl"><span className="font-bold uppercase tracking-widest text-primary">Negocie com a Braza</span><h1 className="mt-3 text-[2.3rem] font-extrabold leading-tight sm:text-5xl md:text-6xl">Venda ou consigne seu veículo com a gente</h1><p className="mt-5 text-lg text-foreground/65">Uma negociação simples, transparente e acompanhada por quem conhece o mercado há mais de 30 anos.</p></div><div className="my-14 grid gap-5 md:grid-cols-3">{[[Camera,'1. Conte sobre o veículo','Preencha os dados e adicione fotos.'],[ClipboardCheck,'2. Avaliamos juntos','Nossa equipe analisa sua proposta.'],[MessageCircle,'3. Conversamos','Entramos em contato para combinar os próximos passos.']].map(([Icon,title,text])=>{const I=Icon as typeof Camera;return <div key={title as string} className="rounded-3xl bg-muted p-7"><I className="text-primary" size={30}/><h2 className="mt-5 text-xl">{title as string}</h2><p className="mt-2 text-sm text-foreground/65">{text as string}</p></div>})}</div>
  {success?<div className="rounded-3xl bg-muted p-10 text-center"><CheckCircle2 className="mx-auto text-primary" size={50}/><h2 className="mt-4 text-3xl">Solicitação enviada!</h2><p className="mt-3">Recebemos seus dados. Nossa equipe entrará em contato.</p><a href={whatsappUrl} target="_blank" rel="noopener noreferrer" className="btn-primary mt-6">Abrir conversa no WhatsApp</a></div>:<form onSubmit={handleSubmit(submit)} className="mx-auto max-w-4xl rounded-3xl border border-border bg-card p-6 md:p-10"><h2 className="mb-7 text-3xl">Fale do seu veículo</h2><div className="grid gap-5 sm:grid-cols-2">
...
function field(_id:string,label:string,control:React.ReactNode,error?:string){return <label className="block"><span className="label">{label}</span>{control}{error&&<span role="alert" className="mt-1 block text-xs text-accent">{error}</span>}</label>}
function PhotoPreview({file,index}:{file:File;index:number}){const [url,setUrl]=useState('');useEffect(()=>{const next=URL.createObjectURL(file);setUrl(next);return()=>URL.revokeObjectURL(next)},[file]);return url?<img src={url} alt={`Prévia ${index+1}`} width="120" height="120" className="aspect-square w-full rounded-xl object-cover"/>:<div className="skeleton aspect-square w-full rounded-xl"/>}
