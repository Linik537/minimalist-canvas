...
export default function Home(){
  ...
  <section className="section-space bg-muted"><div className="container-wide"><div className="mb-10 flex items-end justify-between gap-4"><div><span className="text-sm font-bold uppercase tracking-widest text-primary">Seleção Braza</span><h2 className="mt-2 text-4xl font-extrabold md:text-5xl">Destaques</h2></div><Link to="/estoque" className="btn-primary hidden sm:inline-flex">Ver todos <ArrowUpRight size={18}/></Link></div>{isLoading?<div className="skeleton h-80 rounded-3xl"/>:<Reveal className="grid justify-center gap-6 [grid-template-columns:repeat(auto-fit,minmax(min(100%,20rem),26rem))]">{data.filter(v=>v.featured).slice(0,4).map((v,i)=><VehicleCard key={v.id} vehicle={v} highlight={i===0}/>)}</Reveal>}{!isLoading&&data.filter(v=>v.featured).length===0&&<p className="rounded-2xl bg-card p-8">Em breve, novos destaques por aqui. <Link to="/estoque" className="text-primary underline">Veja todo o estoque</Link>.</p>}</div></section>
  ...
  <section className="banner-stage relative min-h-[460px] text-inverse-foreground"><div className="container-wide flex min-h-[460px] flex-col justify-center py-16"><h2 className="max-w-2xl text-4xl font-extrabold md:text-6xl">Venha conhecer a Braza de perto</h2><p className="mt-4 max-w-lg text-inverse-foreground/80">Estamos em {site.address}. Passe aqui, veja os veículos e converse com a nossa equipe.</p><a href={site.whatsappLink} target="_blank" rel="noopener noreferrer" className="btn-primary mt-8 w-fit">Fale no WhatsApp <ArrowUpRight size={18}/></a></div></section>
  </>
}
