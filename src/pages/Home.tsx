...
export default function Home(){
  ...
  <section className="section-space bg-muted"><div className="container-wide"><div className="mb-10 flex items-end justify-between gap-4"><div><span className="text-sm font-bold uppercase tracking-widest text-primary">Seleção Braza</span><h2 className="mt-2 text-4xl font-extrabold md:text-5xl">Destaques</h2></div><Link to="/estoque" className="btn-primary hidden sm:inline-flex">Ver todos <ArrowUpRight size={18}/></Link></div>{isLoading?<div className="skeleton h-80 rounded-3xl"/>:<Reveal className="grid gap-6 md:grid-cols-2">{data.filter(v=>v.featured).slice(0,4).map((v,i)=><VehicleCard key={v.id} vehicle={v} highlight={i===0}/>)}</Reveal>}{!isLoading&&data.filter(v=>v.featured).length===0&&<p className="rounded-2xl bg-card p-8">Em breve, novos destaques por aqui. <Link to="/estoque" className="text-primary underline">Veja todo o estoque</Link>.</p>}</div></section>
