...
export default function Inventory(){
  ...
  {error?<p className="rounded-2xl bg-muted p-8">Não foi possível carregar o estoque. Tente novamente mais tarde.</p>:isLoading?<div className="skeleton h-80 rounded-3xl"/>:<><p className="mb-5 text-sm text-foreground/60">{filtered.length} veículos encontrados</p><div className="grid justify-center gap-6 [grid-template-columns:repeat(auto-fit,minmax(min(100%,20rem),26rem))]">{filtered.slice(0,limit).map(v=><VehicleCard key={v.id} vehicle={v}/>)}</div>{filtered.length>limit&&<button className="btn-outline mx-auto mt-10 flex" onClick={()=>setLimit(x=>x+12)}>Carregar mais</button>}{filtered.length===0&&<div className="rounded-3xl bg-muted p-12 text-center"><h2 className="text-2xl">Nenhum veículo encontrado</h2><p className="my-4">Podemos ajudar você a encontrar o modelo ideal.</p><a className="btn-primary" href={site.whatsappLink} target="_blank" rel="noopener noreferrer">Fale conosco</a></div>}</>}</div></div></section>
