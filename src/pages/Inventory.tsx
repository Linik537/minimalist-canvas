...
export default function Inventory(){
  ...
  ].map(([key,label,values])=><label key={key as string}><span className="label">{label as string}</span><select className="field" value={get(key as string)} onChange={e=>set(key as string,e.target.value)}>{(values as string[]).map((v,i)=><option value={v} key={i}>{v==='carro'?'Carro':v==='moto'?'Moto':v||'Todos'}</option>)}</select></label>)}
