...
export default function VehicleForm(){
  ...
  const save=async(v:Values)=>{if(busy)return;setBusy(true);setToast(null);setProgress(0);try{
    ...
    else{const {data:created,error}=await supabase.from('vehicles').insert({...payload,slug:vehicleSlug(v.brand,v.model,v.model_year)}).select('id').single();if(error)throw error;vehicleId=created.id;createdId.current=created.id}
    ...
    reset(v);await qc.invalidateQueries({queryKey:['vehicles']});setBusy(false);navigate('/admin',{state:{notice:'Veículo salvo com sucesso.'}})
