...
export default function VehicleForm({userId}:{userId:string}){
  ...
  useEffect(()=>{if(!formReady||!pendingReady)return;const ids=[...photos.map(p=>p.id),...pending.map(p=>p.id)];setOrder(current=>{const next=[...current.filter(x=>ids.includes(x)),...ids.filter(x=>!current.includes(x))];return next.join('|')===current.join('|')?current:next});setSelectedId(current=>current&&ids.includes(current)?current:ids[0]??null)},[formReady,pendingReady,photos,pending])
  ...
  const editorPhotos=useMemo<EditorPhoto[]>(()=>{const items=[...photos.map(p=>({id:p.id,source:p.url,label:`Foto ${p.position+1}`,isNew:false})),...pending.map(p=>({id:p.id,source:p.file,label:p.file.name,isNew:true}))];return items.sort((a,b)=>order.indexOf(a.id)-order.indexOf(b.id))},[photos,pending,order])
  const changeOrder=(next:string[])=>{setOrder(next);setPhotoDirty(true)}
  const movePhoto=(photoId:string,delta:number)=>{const ids=editorPhotos.map(p=>p.id),index=ids.indexOf(photoId),target=index+delta;if(index>=0&&target>=0&&target<ids.length)changeOrder(arrayMove(ids,index,target))}
  ...
  const reorder=(active:string,over:string)=>{if(busy)return;const ids=editorPhotos.map(p=>p.id),from=ids.indexOf(active),to=ids.indexOf(over);if(from>=0&&to>=0)changeOrder(arrayMove(ids,from,to))}
  const deletePhoto=async(photoId:string)=>{if(busy)return;const existing=photos.find(p=>p.id===photoId);if(!existing){setPending(current=>current.filter(p=>p.id!==photoId));setOrder(current=>current.filter(x=>x!==photoId));return}if(!confirm('Remover esta foto do banco e do Storage?'))return;setBusy(true);try{const {error}=await supabase.from('vehicle_photos').delete().eq('id',existing.id);if(error)throw error;setPhotos(p=>p.filter(x=>x.id!==existing.id));setOrder(current=>current.filter(x=>x!==existing.id));const {error:storageError}=await supabase.storage.from('vehicle-photos').remove([existing.storage_path]);await qc.invalidateQueries({queryKey:['vehicles']});setToast({text:storageError?'Foto removida do banco; confira o arquivo antigo no Storage.':'Foto removida.',kind:storageError?'error':'success'})}catch(e){setToast({text:e instanceof Error?e.message:'Não foi possível remover a foto.',kind:'error'})}finally{setBusy(false)}}
  const applyPhoto=async(photoId:string,blob:Blob)=>{const queued=pending.find(p=>p.id===photoId);if(queued){const name=queued.file.name.replace(/\.[^.]+$/,'')+'.webp';setPending(current=>current.map(p=>p.id===photoId?{...p,file:new File([blob],name,{type:'image/webp'})}:p));setCrops(current=>({...current,[photoId]:defaultPhotoCrop}));setToast({text:'Ajuste aplicado à foto nova. Salve o veículo para enviar.',kind:'success'});return}const existing=photos.find(p=>p.id===photoId);if(!existing)return;setBusy(true);const path=`${existing.vehicle_id}/${crypto.randomUUID()}.webp`;try{const {error:uploadError}=await supabase.storage.from('vehicle-photos').upload(path,blob,{contentType:'image/webp'});if(uploadError)throw uploadError;const url=supabase.storage.from('vehicle-photos').getPublicUrl(path).data.publicUrl;const {data:updated,error}=await supabase.from('vehicle_photos').update({url,storage_path:path}).eq('id',photoId).select().single();if(error){await supabase.storage.from('vehicle-photos').remove([path]);throw error}setPhotos(current=>current.map(p=>p.id===photoId?updated:p));setCrops(current=>({...current,[photoId]:defaultPhotoCrop}));const {error:removeError}=await supabase.storage.from('vehicle-photos').remove([existing.storage_path]);await qc.invalidateQueries({queryKey:['vehicles']});setToast({text:removeError?'Foto aplicada; confira o arquivo antigo no Storage.':'Foto ajustada e otimizada.',kind:removeError?'error':'success'})}finally{setBusy(false)}}
  const save=async(v:Values)=>{if(busy)return;setBusy(true);setToast(null);setProgress(0);try{
    ...
    for(const [position,photoId] of ordered.entries()){
      ...
      if(existing){if(existing.position!==position){const {error}=await supabase.from('vehicle_photos').update({position}).eq('id',existing.id);if(error)throw error;setPhotos(current=>current.map(p=>p.id===existing.id?{...p,position}:p))}continue}
      const upload=pending.find(p=>p.id===photoId)
      if(!upload)continue
      const blob=await croppedPhotoBlob(upload.file,crops[photoId]??defaultPhotoCrop)
      ...
      setPhotos(current=>[...current,photoRow]);setPending(current=>current.filter(item=>item.id!==photoId));setOrder(current=>current.map(x=>x===photoId?photoRow.id:x));setProgress(Math.round((position+1)/ordered.length*100))
    }
  ...
  <VehiclePhotoEditor photos={editorPhotos} selectedId={selectedId} crop={crops[selectedId??'']??defaultPhotoCrop} busy={busy} progress={progress} error={photoError} onSelect={setSelectedId} onCrop={crop=>{if(selectedId){setCrops(current=>({...current,[selectedId]:crop}));setPhotoDirty(true)}}} onMove={movePhoto} onCover={photoId=>{const ids=editorPhotos.map(p=>p.id);changeOrder([photoId,...ids.filter(x=>x!==photoId)])}} onRemove={deletePhoto} onReorder={reorder} onApply={applyPhoto} onFiles={addFiles}/><button disabled={busy||!pendingReady||!formReady} className="btn-primary mt-8 disabled:opacity-50">{busy?'Salvando...':'Salvar veículo'}</button></form>}{blocker.state==='blocked'&&<div className="fixed inset-0 z-50 grid place-items-center bg-inverse/60 p-4"><div className="max-w-md rounded-3xl bg-card p-8"><h2 className="text-2xl">Alterações não salvas</h2><p className="my-4">Deseja sair sem salvar?</p><div className="flex gap-3"><button className="btn-outline" onClick={()=>blocker.reset()}>Continuar editando</button><button className="btn-primary" onClick={()=>blocker.proceed()}>Sair</button></div></div></div>}<AdminToast toast={toast} onClose={()=>setToast(null)}/></div>
